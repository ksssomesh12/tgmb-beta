...
def stageZero(update: telegram.Update, _: telegram.ext.CallbackContext) -> int:
    ...
    if isValidDl:
        ...
        mirrorInfo.googleDriveUploadFolderId = list(botHelper.configHelper.configVars[botHelper.configHelper.reqVars[5]].keys())[0]
        # </setDefaults>
...
def stageTwo(update: telegram.Update, _: telegram.ext.CallbackContext) -> int:
    ...
    query.answer()
    if query.data == '1':
        buttonList = [*list(botHelper.configHelper.configVars[botHelper.configHelper.reqVars[5]].values())]
...
def stageThree(update: telegram.Update, _: telegram.ext.CallbackContext) -> int:
    query = update.callback_query
    query.answer()
    mirrorInfo.googleDriveUploadFolderId = list(botHelper.configHelper.configVars[botHelper.configHelper.reqVars[5]].keys())[(int(query.data) - 1)]
    buttonList = ['isCompress', 'isDecompress', 'Skip']
    query.edit_message_text(text='Choose:', reply_markup=InlineKeyboardMaker(buttonList).build(1))
    return FOURTH
